...
public:
  ...
  char *idaapi get_node_label(char *buf, int bufsize, int n) const
  {
    char *ptr = buf;
...
static const action_desc_t display_graph_action = ACTION_DESC_LITERAL(
        ACTION_NAME,
        "Display graph",
        &display_graph_ah,
        ACTION_SHORTCUT,
...
int idaapi init(void)
{
  ...
  register_action(display_graph_action);
  inited = true;
  return PLUGIN_KEEP;
}
